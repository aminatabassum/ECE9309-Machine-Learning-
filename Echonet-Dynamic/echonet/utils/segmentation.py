...
def run(
    ...
    # Set up optimizer
    optim = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)  # Use AdamW optimizer
    if lr_step_period is None:
        lr_step_period = math.inf
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optim, T_max=num_epochs)  # Use cosine annealing
    ...
    with open(os.path.join(output, "log.csv"), "a") as f:
        ...
        for epoch in range(epoch_resume, num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                loss, large_inter, large_union, small_inter, small_union = echonet.utils.segmentation.run_epoch(
                    model, dataloader, phase == "train", optim, device, gradient_clip=1.0)  # Add gradient clipping
...
def run_epoch(model, dataloader, train, optim, device, gradient_clip=None):
        ...
        with tqdm.tqdm(total=len(dataloader)) as pbar:
            for (_, (large_frame, small_frame, large_trace, small_trace)) in dataloader:
                ...
                if train:
                    optim.zero_grad()
                    loss.backward()
                    if gradient_clip:  # Apply gradient clipping
                ...
                pbar.set_postfix_str("{:.4f} ({:.4f}) / {:.4f} {:.4f}, {:.4f}, {:.4f}".format(total / n / 128 / 128, loss.item() / large_trace.size(0) / 128 / 128, -p * math.log(p) - (1 - p) * math.log(1 - p), (-p_pix * np.log(p_pix) - (1 - p_pix) * np.log(1 - p_pix)).mean(), 2 * large_inter / (large_union + large_inter), 2 * small_inter / (small_union + small_inter)))
                pbar.update()
